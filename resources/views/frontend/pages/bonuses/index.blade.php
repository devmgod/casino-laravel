...
@section('content')
    @if($faucet['config']['amount'] > 0)
        <h2>{{ __('Faucet') }}</h2>
        <p>
            {{ __('You can claim :n credits every :x hours.', ['n' => $faucet['config']['amount'], 'x' => $faucet['config']['interval']]) }}
        </p>
        @if($faucet['allowed'])
            <form method="POST" action="{{ route('frontend.bonuses.faucet') }}">
                @csrf
                <button class="btn btn-primary" type="submit">
                    <i class="fa fa-coins"></i>
                    {{ __('Claim :n credits', ['n' => $faucet['config']['amount']]) }}
                </button>
            </form>
        @else
            <span>{{ __('Wait time for the next faucet') }}</span>
            &mdash;
            <countdown-timer end-date="{{ $faucet['time'] }}"></countdown-timer>
        @endif
    @endif

    <h2 class="mt-4">{{ __('Referral program') }}</h2>
    <p>
